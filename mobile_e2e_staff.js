...
const testStaff = async () => {
    console.log('Starting Full Mobile E2E Test as Staff Member...');
    ...
    try {
        ...
        if (isRolePage) {
            const roleCard = await page.locator('ion-card', { hasText: 'Staff Member' });
            if (await roleCard.count() > 0) await roleCard.first().click();
            await page.locator('ion-button', { hasText: 'Continue' }).click();
            await page.waitForTimeout(1000);
        }

        await page.fill('input[type="email"], ion-input[type="email"] input', 'staff1@e2etesting.com');
        ...
        console.log('2. Verifying Staff Dashboard...');
        await page.waitForTimeout(4000);
        await page.screenshot({ path: path.join(outDir, 'mobile_staff_01_dashboard.png') });

        console.log('3. Navigating to Tasks (Restricted)...');
        await page.goto('http://localhost:8100/tasks');
        await page.waitForTimeout(3000);
        await page.screenshot({ path: path.join(outDir, 'mobile_staff_02_tasks.png') });

        console.log('4. Navigating to Complaints View directly (Should be blocked/empty)...');
        await page.goto('http://localhost:8100/complaints');
        await page.waitForTimeout(3000);
        await page.screenshot({ path: path.join(outDir, 'mobile_staff_03_complaints_blocked.png') });

        console.log('5. Navigating to Settings directly (Should be restricted out)...');
        await page.goto('http://localhost:8100/settings');
        await page.waitForTimeout(3000);
        await page.screenshot({ path: path.join(outDir, 'mobile_staff_04_settings_blocked.png') });

        console.log('✅ ALL STAFF E2E SCENARIOS CAPTURED!');
